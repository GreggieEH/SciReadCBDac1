...
CPlotWindow::CPlotWindow(CDlgRunCBDac * pDlgRunCBDac) :
	CMyContainer(),
...
{
	IDispatch	*	pdispContainer;
	HRESULT			hr;
	hr = this->QueryInterface(IID_IDispatch, (LPVOID*)&pdispContainer);
	if (SUCCEEDED(hr))
	{
		Utils_InvokeMethod(pdispContainer, DISPID_Container_CloseAllObjects, NULL, 0, NULL);
		pdispContainer->Release();
	}
}
	...
	VARIANT		*	Parameters)
{
	if (0 == lstrcmpi(ObjectName, PLOT_WINDOW))
	{
		if (DISPID_MOUSEMOVE == Dispid && 4 == nParams)
		{
			...
			this->m_pDlgRunCBDac->OnMouseMove(x, y);
		}
	}
}
